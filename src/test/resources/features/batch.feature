 ...
 Feature: Batch feature
  ...
  @batch_page_validation
	...
	Scenario: Validate edit icon in each data rows
		Given Admin is on the home page
		When Admin Clicks on the Batch menu from the header
		ThenAdmin should see the edit icon in each row
	
	@batch_page_validation_6
	Scenario: Validate delete icon in each data rows
		Given Admin is on the home page
		When Admin Clicks on the Batch menu from the header
		ThenAdmin should see the delete icon in each row
	
	@batch_page_validation_7
	Scenario: Validate checkbox in each data rows
		Given Admin is on the home page
		When Admin Clicks on the Batch menu from the header
		ThenAdmin should see the checkbox in each row
	
	@batch_page_validation_8
	Scenario: Validate Datatable headers
		Given Admin is on the home page
		When Admin Clicks on the Batch menu from the header
		ThenAdmin should see the datatable headers Batch name, Batch Description,Batch Status, No Of classes, Program Name, Edit/Delete
	
	@batch_page_validation_9
	Scenario: Validate "Checkbox" in the Datatable header row
		Given Admin is on the home page
		When Admin Clicks on the Batch menu from the header
		ThenAdmin should see the checkbox  in the datatable header row
	
	@batch_page_validation_10
	Scenario: Validate "sort icon" next to all the datatable header
		Given Admin is on the home page
		When Admin Clicks on the Batch menu from the header
		ThenAdmin should see the sort icon next to all Datatable headers
	...
	@batch_add_new_batch_details_popup		
